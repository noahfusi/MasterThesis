...
object Main {
  ...
  def updatePin(machineId: Int, machinePins: Array[String]): Unit = {
    ...
    if (newcode.length == 6) {
      ...
      println("Le nouveau code est: " + machinePins(machineId))
...
while (boucle) {
  ...
  while (boisson != 1 && boisson != 2 && boisson != 3 ) {
    ...
    Thread.sleep(2000)
    if (coffeeStocks(machineId) < qcafe) {
    ...
    else if (milkStocks(machineId) < qmilk) {
    ...
    else if (sugarStocks(machineId) < qsugar) {
      ...
      coffeeStocks(machineId) = coffeeStocks(machineId) - qcafe
      milkStocks(machineId) = milkStocks(machineId) - qmilk
      sugarStocks(machineId) = sugarStocks(machineId) - qsugar
  ...
  def restockMachine(machineId: Int, coffeeStocks: Array[Int],
    ...
    println("Poudre à café: " + coffeeStocks(machineId) + "g")
    ...
    coffeeStocks(machineId) = coffeeStocks(machineId) + ajoutcoffee
    val ajoutsugar = readLine("Sucre: ").toInt
    sugarStocks(machineId) = sugarStocks(machineId) + ajoutsugar
    val ajoutmilk = readLine("Lait: ").toInt
    milkStocks(machineId) = milkStocks(machineId) + ajoutmilk
  ...
  def main(args: Array[String]): Unit = {
    ...
    while (mode != 3) {
      ...
      if (mode == 1) {
        ...
        if (machine == 1 || machine == 2 || machine == 3 || machine == 4 || machine == nbMachines) {
          val commande = serveClient(machineId, coffeeStocks, sugarStocks, milkStocks)
          if (commande){
            println("")
          }
          else{
      ...
      if (mode == 2) {
        ...
        val access = validatePin(machineId, machinePins)
        while (essai > 0) {
          ...
          if (access) {
            ...
            if (essai == 2) {
              ...
              val deuxieme = validatePin(machineId, machinePins)
              if (deuxieme) {
                println("Accès autorisé")
                essai = 0
                choix = 0
            ...
            if (essai == 1){
              ...
              val troisieme = validatePin(machineId, machinePins)
              if (troisieme) {
                println("Accès autorisé")
                essai = 0
                choix = 0
