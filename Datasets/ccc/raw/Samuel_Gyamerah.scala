...
object Main {
  ...
  def main(args: Array[String]): Unit = {
    ...
    var ajout_lait = 0.0
    val ajout_petit_lait = 50
    val ajout_moyen_lait = 100
    val ajout_grand_lait = 150
    ...
    def loadcsv(filename: String): ArrayBuffer[Machine] = {
      ...
      try {
        val csv_ligne = Source.fromFile(filename).getLines().drop(1)
        var ID_nombre = 1
        for (i<-csv_ligne){
          val csv_index = i.split(",")

          if(csv_index.length==4) {
            val id = ID_nombre
            machines += new Machine(id, csv_index(0), csv_index(1).toInt, csv_index(2).toInt, csv_index(3).toInt)
          }
          ID_nombre += 1
        }
      }catch {
    ...
    def savecsv(filename: String, machines: ArrayBuffer[Machine]): Unit = {
      try{
        val ecriture = new PrintWriter( filename)
        ecriture.println("PINCODE,MILK,SUGAR,COFFEE")

        for(index<-machines){
          val csv_ligne = index.pincode+","+index.milk+","+index.sugar+","+index.coffee
          ecriture.println(csv_ligne)
        }

        ecriture.close()
        println("Sauvegarde de 5 machines dans machines.csv... \nFichier sauvegard´e avec succ`es.")
      }catch {
        case _: IOException =>
          println("Erreur : ´Echec de l’écriture dans machines.csv. \nLe fichier peut ^etre verrouillé ou en lecture seule.")
      }
    }
    def validatePin(machineId: Int, machines: ArrayBuffer[Machine]):Boolean = {
      val code = readLine(">")
      if (machines(machineId).pincode == code) {
        true
      }
      else {
        false
    ...
    def serveClient(machineId: Int, machines:ArrayBuffer[Machine]): Boolean ={
      ...
      if (machines(machineId).coffee < ajout_cafe){
      ...
      if (machines(machineId).sugar < sucre_quantite_ajout){
        println("Erreur : Quantite de sucre insuffisante pour\npreparer la boisson selectionnee.\nVeuillez choisir une autre boisson ou verifier les\nstocks en mode Admin.")
      }
      if (machines(machineId).milk < quantite_tot_lait){
      ...
      if ((machines(machineId).coffee > ajout_cafe) && (machines(machineId).sugar > sucre_quantite_ajout) && (machines(machineId).milk > lait_quantite_ajout)) {
        ...
        machines(machineId).removeIngredient("cafe",cafe_utilise.toInt)
        machines(machineId).removeIngredient("lait",lait_quantite_ajout + lait_utilise )
        machines(machineId).removeIngredient("sucre",sucre_quantite_ajout.toInt)
        mode = 0
        lait_ajout_supp = 0
        true
      }
    ...
    def restockMachine(machineId: Int, machines: ArrayBuffer[Machine]): Unit ={
      ...
      var lait_rajout = readLine("\tLait :").toInt
      while (lait_rajout <0){
        lait_rajout = readLine("\tLait :").toInt
      }
      ...
      machines(machineId).addIngredient("cafe",cafe_rajout.toInt)
      machines(machineId).addIngredient("sucre",sucre_rajout.toInt)
      machines(machineId).addIngredient("lait",lait_rajout)
    }
    // initialiser le tableau Machine
    val filename = "machines.csv"
    val machines = loadcsv(filename)

    for(i<-machines){
      println(s"Machine ${i.id} chargée: ")
      println(s"\tID: ${i.id}")
      println(s"\tPin: ${i.pincode}")
      println(s"\tLait: ${i.milk}")
      println(s"\tSucre: ${i.sugar}")
      println(s"\tCafé: ${i.coffee}")
    }
    ...
    while (forwhile != 1) {
      ...
      while (mode == 2) {
        ...
        while ((!validatePin(machineID, machines)) && (tentatives >1)) {
          tentatives -= 1
