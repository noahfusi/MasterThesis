...
object Exercice2 {
  ...
  def updatePin(machineId: Int, machinePins: Array[String]): Unit = {
    ...
    machinePins(machineId) = nv_code_pin
  ...
  def serveClient(machineId: Int, coffeeStocks: Array[Int], sugarStocks: Array[Int],
    ...
    if (boisson == 1) {
      res_cafe = coffeeStocks(machineId) - achat_expresso_cafe
      res_lait = milkStocks(machineId)
      res_prix = res_prix + prix_expresso
      prix_boisson_simple = prix_expresso
      println("\nBoisson sélectionnée : Expresso")

    } else if (boisson == 2) {
      res_cafe = coffeeStocks(machineId) - achat_cappuccino_cafe
      res_lait = milkStocks(machineId) - achat_cappuccino_lait
      res_prix = res_prix + prix_cappuccino
      prix_boisson_simple = prix_cappuccino
      println("\nBoisson sélectionnée : Cappuccino")

    } else if (boisson == 3) {
      res_cafe = coffeeStocks(machineId) - achat_latte_petit_cafe
      res_lait = milkStocks(machineId) - achat_latte_petit_lait
      res_prix = res_prix + prix_latte_petit
      prix_boisson_simple = prix_latte_petit
      println("\nBoisson sélectionnée : Latte (Petit)")

    } else if (boisson == 4) {
      res_cafe = coffeeStocks(machineId) - achat_latte_moyen_cafe
      res_lait = milkStocks(machineId) - achat_latte_moyen_lait
      res_prix = res_prix + prix_latte_moyen
      prix_boisson_simple = prix_latte_moyen
      println("\nBoisson sélectionnée : Latte (Moyen)")

    } else if (boisson == 5) {
      res_cafe = coffeeStocks(machineId) - achat_latte_grand_cafe
      res_lait = milkStocks(machineId) - achat_latte_grand_lait
    ...
    if (!erreur) {
      ...
      coffeeStocks(machineId) = res_cafe
      milkStocks(machineId) = res_lait
      sugarStocks(machineId) = res_sucre
    }
  ...
  def restockMachine(machineId: Int, coffeeStocks: Array[Int], sugarStocks: Array[Int],
                     milkStocks: Array[Int]): Unit = {
    ...
    coffeeStocks(machineId) = coffeeStocks(machineId) + ajout_cafe
    milkStocks(machineId) = milkStocks(machineId) + ajout_lait
    sugarStocks(machineId) = sugarStocks(machineId) + ajout_sucre
  ...
  def main(args: Array[String]): Unit = {
    ...
    // les 4 tableaux du début
    val tab_stock_lait = Array.fill(nbMachines)(500)
    val tab_stock_cafe = Array.fill(nbMachines)(50)
    val tab_stock_sucre = Array.fill(nbMachines)(30)
    val tab_pin_code = Array.fill(nbMachines)("434343")
    ...
    while (onoff) {
      ...
      if (mode == 1) {
        ...
        do {
          nbr_machine = (readLine("\nMachine sélectionnée (1-5) :").toInt)-1

        } while ((nbr_machine < 0) || (nbr_machine > nbMachines-1))

        etat_achat = serveClient(nbr_machine, tab_stock_cafe, tab_stock_sucre, tab_stock_lait)
        ...
        do {
          ...
          etat_pin = validatePin(nbr_machine, tab_pin_code)
        ...
        if (etat_pin) {
          ...
          if (action_admin == 1) {
            updatePin(nbr_machine, tab_pin_code)
          } else if (action_admin == 2){
            restockMachine(nbr_machine, tab_stock_cafe, tab_stock_sucre, tab_stock_lait)
