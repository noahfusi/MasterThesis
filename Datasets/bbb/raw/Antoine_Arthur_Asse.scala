...
object Main {
  ...
  def validatePin(machineId: Int, machinePins: Array[String]): Boolean = {
    ...
    if (machinePins(machineId-1)==codepinpardefaut){  //vérifier qu'il correspond à celui de la machine séléctionnée
  ...
  def serveClient(machineId: Int, coffeeStocks: Array[Int], sugarStocks: Array[Int], milkStocks: Array[Int]): Boolean = {
    ...
    if(laitdesire>milkStocks(machineId-1))
    {
      bloque1=true
      println("")
    ...
    if(sucredesire>sugarStocks(machineId-1))
    {
      bloque2=true
      println("")
    ...
    if(cafedesire>coffeeStocks(machineId-1))
    {
      println("")
      bloque3=true
    ...
    if(!bloque1 && !bloque2 && !bloque3)
    {
      ...
      sugarStocks(machineId-1) = sugarStocks(machineId-1) - sucredesire
      coffeeStocks(machineId-1) = coffeeStocks(machineId-1) - cafedesire
      milkStocks(machineId-1) = milkStocks(machineId-1) - laitdesire
      var codealeatoire=""
  ...
  def updatePin(machineId: Int, machinePins: Array[String]): Unit = {
    ...
    machinePins(machineId-1) = changementdepin  //Stockage du code
  ...
  def restockMachine(machineId: Int, coffeeStocks: Array[Int], sugarStocks: Array[Int], milkStocks: Array[Int]): Unit = {
    ...
    print("Sucre : "+sugarStocks(machineId-1))
    println("g")
    var affichagelait=milkStocks(machineId-1)/1000.0  //création d'une variable qui stocke le lait actuel mais divisé par 1000
    ...
    sugarStocks(machineId-1) += sucreplus
    milkStocks(machineId-1) += laitplus.toInt //conversion en entier
    coffeeStocks(machineId-1) += poudreplus
    ...
    val nbmachines=5  //Consigne du sujet : une variable qui définit le nombre de machines
    //Tableaux de stocks pour chaque ingrédient et qui contient les machines
    var departsucre= Array.fill(nbmachines)(30)
    var departlait=Array.fill(nbmachines)(500)
    var departcafe=Array.fill(nbmachines)(50)

    var rester="oui"

    val admin=Array.fill(nbmachines)("434343")  //tableaux de pin (string)
    while(rester=="oui")  //tant que la variable rester vaut oui, on reste dedans, lorsqu'elle vaudra non, il faudra quitter le programme
    {
      ...
      { //Si l'utilisateur rentre en mode client, on affiche la séléction des boissons
        ...
        var reussit = serveClient(machine_saisie,departcafe,departsucre,departlait)
      ...
      {
        println("Mode Admin")
        while(machine_saisie<1 || machine_saisie>nbmachines){
          print("Machine sélectionnée (1-5) > ")
          machine_saisie = readInt()
        }
        var tentatives=3  //Commencer avec 3 essais
        while(validatePin(machine_saisie,admin)==false){  //tant que la fonction retourne faux (mauvais pin), on boucle
        ...
        if(rester!="non"){
          ...
          if(option_selectionnee==1){
            restockMachine(machine_saisie,departcafe,departsucre,departlait)
          }
          else{
            updatePin(machine_saisie,admin)
          }
        }
      }//fin du mode admin
      else if(decision==3)
