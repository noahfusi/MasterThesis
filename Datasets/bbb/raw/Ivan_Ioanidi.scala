...
object Main {
  def validatePin(machineId: Int, machinePins: Array[String]): Boolean = {
    ...
    if (machinePins(machineId) == pinUtil) {
      return true
    } else {
      return false
    }
  ...
  def updatePin(machineId: Int, machinePins: Array[String]): Unit = {
    var nouveauPin = readLine("Entrez un nouveau code PIN a 6 chiffres > ")
    while(nouveauPin.length != 6) {
      nouveauPin = readLine("Entrez un nouveau code PIN a 6 chiffres > ")
    }
  ...
  def serveClient(machineId: Int, coffeeStocks: Array[Int], sugarStocks: Array[Int], milkStocks: Array[Int]): Boolean ={
    ...
    if ((boisson == 1) || (boisson == 2)) {
      if (laitNecess > milkStocks(machineId)) {
      ...
      if (cafeNecess > coffeeStocks(machineId)) {
        println(" Erreur : Quantité de poudre de café insuffisante pour préparer la boisson sélectionnée.")
        println("Veuillez choisir une autre boisson ou vérifier les stocks en mode Admin.")
        println("Veuillez choisir une autre machine.")
        sansErreur = false
      }
      if (sucreNecess > sugarStocks(machineId)) {
    ...
    //Gestion d'erreur pour la boisson 3
      ...
      else if (cafeNecess > coffeeStocks(machineId)) {
        println("Erreur : Quantité de poudre de café insuffisante pour préparer la boisson sélectionnée.")
        println("Veuillez choisir une taille plus petite ou essayer une autre boisson.")
        println("Veuillez choisir une autre machine.")
        sansErreur = false
      }
      else if (sucreNecess > sugarStocks(machineId)) {
    ...
    coffeeStocks(machineId) = coffeeStocks(machineId) - cafeNecess
    milkStocks(machineId) = milkStocks(machineId) - laitNecess
    sugarStocks(machineId) = sugarStocks(machineId) - sucreNecess
  ...
  def restockMachine(machineId: Int, coffeeStocks: Array[Int], sugarStocks: Array[Int], milkStocks: Array[Int]): Unit = {
    ...
    coffeeStocks(machineId) = coffeeStocks(machineId) + coffee_add
    val milk_add = readLine("  Lait          : ").toInt
    milkStocks(machineId) = milkStocks(machineId) + milk_add
    val sugar_add = readLine("  Sucre         : ").toInt
    sugarStocks(machineId) = sugarStocks(machineId) + sugar_add
  ...
  def main(args: Array[String]): Unit = {
    ...
    //On définie le nombre de machines comme une valeur
    val nbMachines = 5
    //var identifiant = None

    //Stocks sont utilisé dans les deux modes donc variab le globale
    // initialise les stocks en Array

    val stockCafe = Array.fill(nbMachines)(50) // en g
    val stockSucre = Array.fill(nbMachines)(30) // en g
    val stockLait = Array.fill(nbMachines)(500) // en ml
    ...
    while (reaffiche) {
      ...
      if (mode == 1) {
        ...
        machineId -=1
        val serve = serveClient(machineId, stockCafe, stockSucre, stockLait)
      ...
      //Mode admin
        ...
        val machinePins = Array.fill(nbMachines)("434343") //default Pin code
        println("Mode Admin")
        var machineId = readLine("Machine sélectionnée (1-5) > ").toInt
        while ((machineId > 5) || (machineId < 1)) {
          machineId = readLine("Machine sélectionnée (1-5) > ").toInt
        ...
        var validite = validatePin(machineId, machinePins)
        var tentative = 3
        while ((tentative != 0 ) && (validite == false)) {
          tentative -= 1
          validite = validatePin(machineId, machinePins)
        }
