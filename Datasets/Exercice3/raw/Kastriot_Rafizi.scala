...
import scala.collection.mutable.ArrayBuffer // librairie pour les arrays buffer
import java.io.{FileWriter,PrintWriter} // afin de lire et écrire dans un fichier
...
class Machine(val idMachine: Int, var pincodeMachine: String, var milkMachine: Int, var sugarMachine: Int, var coffeMachine: Int){

 // attributs
  val id = idMachine
  var pincode = pincodeMachine
  var milk = milkMachine
  var sugar = sugarMachine
  var coffe = coffeMachine


  //méthodes
    //méthode pour teste l'affichage des attributs de la classe
  def affiche_infos()={
    println("ID : " + id)
    println("Code PIN : " + pincode)
    printf("Lait  : %.3fL\n", milk/1000.0)
    println("Sucre : " + sugar + "g")
    println("Café  : " + coffe + "g")
  }

  def addIngredient(ingredient: String, amount: Int): Unit = {
    //je traite le cas où il n'y a que 3 possibilités, je forcerais l'utilisateur à faire l'un de ces chhoix
    if(ingredient == "cafe"){
      coffe += amount
    }
      ...
      println("Erreur ingrédient")
    }
  } ///////////// fin de la méthode addIngredient

  def removeIngredient(ingredient: String, amount: Int): Boolean = {
    var etatOperation = false
    if(ingredient == "cafe"){
      if(coffe>=amount){
        coffe-=amount
        etatOperation=true
  ...
  }/////////// fin de la méthode removeIngredient
...
object Main {
  ...
  //variables de l'exo 1 concernant le mode client :
  ...
  // fin des variables du mode client


  // variables de l'exo 2 principalement pour le mode admin
  ...
  // fin des variables de l'exo 2




  /////////////// métodes implémentées pour l'exo 2

  // vérification du code pin corrigée pour l'exo 3
  def validatePin(machineID: Int, machines: ArrayBuffer[Machine]): Boolean = {
    print("Entrez le code PIN : > ")
    tentativePIN = readLine()
    if (tentativePIN == machines(machineID-1).pincode){
      return true
    }
    else{
      println("Erreur")
  ...
  def updatePin(machineID: Int, machines: ArrayBuffer[Machine]): Unit = {
    ...
    println(s"\nMise à jour du code PIN de la machine ${machines(machineID-1).id}: ")
  ...
  def restockMachine(machineID: Int, machines: ArrayBuffer[Machine]): Unit={
    ...
    println(s"Niveaux des stocks actuels de la machine ${machineID}: ")
    println("\tPoudre de café : " + machines(machineID-1).coffe + "g")
    println("\tSucre          : " + machines(machineID-1).sugar + "g")
    printf("\tLait           : %.3fL\n", (machines(machineID-1).milk)/1000.0 ) // pour afficher en litre
    ...
    machines(machineID-1).addIngredient("cafe", ajout_cafe)
    machines(machineID-1).addIngredient("sucre", ajout_sucre)
    machines(machineID-1).addIngredient("lait",(ajout_lait*1000).toInt)
...
////// CLIENT
  ...
  def serveClient(machineID: Int, machines: ArrayBuffer[Machine]): Boolean = {
    ...
    if(boisson == 1){ // cas expresso
      ...
      val cafe_retire = machines(machineID-1).removeIngredient("cafe", cafe_necessaire)
      val sucre_retire = machines(machineID-1).removeIngredient("sucre", sucre_ajout)

      if (!cafe_retire && !sucre_retire){
      ...
      else if (!cafe_retire && sucre_retire){
        println("Quantité de café insuffisante pour préparer la boisson sélectionnée.\nVeuillez choisir une autre boisson ou vérifier les stocks en mode Admin.\n")
        //remove aura enlevé le sucre quand même donc je rajoute le sucre enlevé avec addIngredient
        machines(machineID-1).addIngredient("sucre", sucre_ajout)
        preparation = false
      }
      else if (cafe_retire && !sucre_retire){
        println("Quantité de sucre insuffisante pour préparer la boisson sélectionnée.\nVeuillez choisir une autre boisson ou vérifier les stocks en mode Admin.\n")
        machines(machineID-1).addIngredient("cafe", cafe_necessaire)
        preparation = false
      }
      else{
        preparation = cafe_retire && sucre_retire
      }
      ...
      if (boisson == 2){ // cas cappuccino
        ...
        val cafe_retire = machines(machineID-1).removeIngredient("cafe", cafe_necessaire)
        val sucre_retire = machines(machineID-1).removeIngredient("sucre", sucre_ajout)
        val lait_retire = machines(machineID-1).removeIngredient("lait", lait_necessaire)


        if (!cafe_retire && !sucre_retire && !lait_retire){ // on a 8 cas
          println("Quantité de café, de sucre et de lait insuffisante pour préparer la boisson sélectionnée.\nVeuillez choisir une autre boisson ou vérifier les stocks en mode Admin.\n")
          preparation = false
        }
        else if (!cafe_retire && !sucre_retire && lait_retire){
          println("Quantité de café et de sucre insuffisante pour préparer la boisson sélectionnée.\nVeuillez choisir une autre boisson ou vérifier les stocks en mode Admin.\n")
          machines(machineID-1).addIngredient("lait",lait_necessaire)
          preparation = false
        }
        else if (!cafe_retire && sucre_retire && !lait_retire){
          ...
          preparation = cafe_retire && sucre_retire && lait_retire
        }
        ...
        if (!cafe_retire && !sucre_retire && !lait_retire){
          println("Quantité de café, de sucre et de lait insuffisante pour préparer la boisson sélectionnée.\nVeuillez choisir une autre boisson ou vérifier les stocks en mode Admin.\n")
          preparation = false
        }
        else if (!cafe_retire && !sucre_retire && lait_retire){
        ...
        else if (cafe_retire && sucre_retire && !lait_retire){ // on propose une autre taille que lorsque le café et le sucre sont dispo mais pas le lait (si pas de café alors ça ne sert à rien
          if (taille_latte==1){
            println("Quantité de lait insuffisante pour préparer la boisson sélectionnée.\nVeuillez choisir une autre boisson ou vérifier les stocks en mode Admin.\n")
          }
          else {
...
//////////////////////////////////////////////////// fin méthodes client
  ...
  /////////////// fin méthodes de l'exo 2



  // les méthodes à implémenter pour la lecture et l'écriture de fichier

  def loadcsv(filename: String): ArrayBuffer[Machine]={

    try {
      var machine = new ArrayBuffer[Machine]()
      val fichier = Source.fromFile(filename)
      val lignefichier = fichier.reset.getLines

      var ligne = lignefichier.next //on saute l'en-tête
      var idMach = 1

      while (!lignefichier.isEmpty) {
        ligne = lignefichier.next

        var machine_infos = ligne.split(",")

        machine += new Machine(idMach, machine_infos(0), machine_infos(1).toInt, machine_infos(2).toInt, machine_infos(3).toInt)
        idMach+=1
      }
      fichier.close()
      return machine
      ...
      case ex : java.lang.NumberFormatException => println("Erreur de conversion d'une chaîne de caractère en nombre.")
        return null
    }
  ...
  var etat_sauvegarde = true // me permet de controler le message d'erreur lorsque je quitte et à chaque manipulation je ne veux pas que le message d'erreur apparaisse
  ...
  def savecsv(filename: String, machines: ArrayBuffer[Machine]): Unit = {
    ...
    try{
      ...
      fw.println("CODEPIN" + "," + "MILK" + "," + "SUGAR" + "," + "COFFEE")
      for (i <- 0 to machines.size - 1) {
        fw.println(machines(i).pincode + "," + machines(i).milk + "," + machines(i).sugar + "," + machines(i).coffe)
      }
      ...
      case ex : java.io.FileNotFoundException => etat_sauvegarde = false

    }
  }//// fin de la méthode save


/////////////// fin des méthodes pour exo 3


  def main(args: Array[String]): Unit = {
    var nom_fichier = "machines.csv"

    println("Chargement des machines depuis machines.csv...\n")
    var machines = loadcsv(nom_fichier)
    if(machines != null) {
      for (i <- 0 to machines.size - 1) {
        println(s"Machine ${i + 1} chargée : \n")
        machines(i).affiche_infos()
        println("\n")
      }
      ...
      do {
        ...
        if (menu == 2) {
          println("Mode Admin")
          print(s"Machine sélectionnée (1-${machines.size}) > ")
          machineChoisie = readInt()
          while (machineChoisie < 1 || machineChoisie > machines.size) {
            println("Erreur")
            print(s"Machine sélectionnée (1-${machines.size}) > ")
            machineChoisie = readInt()
          }
          ...
          while (code_valid == false && tentative > 0) {
            code_valid = validatePin(machineChoisie, machines)
            tentative -= 1
          ...
          if (code_valid == false) {
            ...
            if (choixAdmin == 1) {
              ...
              restockMachine(machineChoisie, machines)
            }
            else {
              //cas où on change le code PIN
              updatePin(machineChoisie, machines)
            }
            println("Retour au menu principal...\n")
          }

          tentative = 3 // on réinitialise le nbre de tentatives pour le code pin et le test de la validité du pin
          code_valid = false
          val filenameWrite = "machines.csv"
          savecsv(filenameWrite, machines)
          ...
          }while(!serveClient(machineChoisie, machines))
        ...
        else { // cas où on quitte
          val filenameWrite = "machines.csv"
          println(s"Sauvegarde de ${machines.size} machine(s) dans " + filenameWrite + "...")
          Thread.sleep(1000) //simulation attente
          savecsv(filenameWrite, machines)
          if(etat_sauvegarde==true){
            println("Fichier sauvegardé avec succès.")
            println("Vous quittez le programme...")

          }
          else{
            println("Échec de l'écriture dans " + filenameWrite)
