...
class Machine(val id: Int, var pincode: String, var milk: Int, var sugar: Int, var coffee: Int) {
  ...
  def addIngredient(ingredient: String, amount: Int): Unit = {
    if (ingredient == "milk") {
      ...
      println(" Faites attention parce que vous avez mis un ingredient inconnu ")
    }
  }
  ...
  def removeIngredient(ingredient: String, amount: Int): Boolean = {
    ...
    if (!updated) {
      println("Quantité insuffisante ou ingrédient inconnu")
    }
    updated
  }
  ...
  def loadcsv(filename: String): ArrayBuffer[Machine] = {
    ...
    try {
      ...
      while (linesIterator.hasNext) {
        val currentLine = linesIterator.next()
        val fields = currentLine.split(",")
        if (fields.length == 4) {
          val code = fields(0)
          val milkAmount = fields(1).toInt
          val sugarAmount = fields(2).toInt
          val coffeeAmount = fields(3).toInt

          machines.append(new Machine(counter, code, milkAmount, sugarAmount, coffeeAmount))
          counter += 1
        }
      }
      fileSource.close()
      ...
      case error: Exception =>
        println(s"Erreur : Impossible de lire le fichier $filename.Le fichier est introuvable")
        sys.exit(1)
    }
    machines
  }

  def savecsv(filepath: String, machineList: ArrayBuffer[Machine]): Unit = {
    try {
      val fileWriter = new PrintWriter(filepath)
      fileWriter.println("PINCODE,MILK,SUGAR,COFFEE")

      var index = 0
      while (index < machineList.size) {
        val machine = machineList(index)
        fileWriter.println(s"${machine.pincode},${machine.milk},${machine.sugar},${machine.coffee}")
        index += 1
      }
      fileWriter.close()
      println(s"Les informations des machines ont été correctement sauvegardées dans le fichier : $filepath.")
    } catch {
      case error: java.io.IOException =>
        println(s"Erreur : Le fichier $filepath est en lecture seule ou verrouillé.")
        sys.exit(1)
      case unexpectedError: Exception =>
        println("Erreur inattendue : Échec de la sauvegarde des machines.")
        sys.exit(1)
    }
  }
